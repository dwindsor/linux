...
int bpf_cred_storage_get(struct bpf_map *map,
			  struct cred *cred,
			  struct bpf_dynptr *value__uninit,
			  void *init,
			  int init__sz,
			  u64 flags);
...
__bpf_kfunc int bpf_cred_storage_get(struct bpf_map *map,
				      struct cred *cred,
				      struct bpf_dynptr *value__uninit,
				      void *init,
				      int init__sz,
...
{
	struct bpf_local_storage_data *sdata;
	struct bpf_dynptr_kern *value_ptr = (struct bpf_dynptr_kern *)value__uninit;
	u32 value_size;
	...
	if (sdata) {
		value_size = ((struct bpf_local_storage_map *)map)->map.value_size;
		bpf_dynptr_init(value_ptr, sdata->data, BPF_DYNPTR_TYPE_LOCAL, 0, value_size);
		return 0;
	}
