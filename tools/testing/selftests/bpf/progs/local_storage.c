...
extern int bpf_dynptr_is_null(const struct bpf_dynptr *ptr);
extern struct bpf_dynptr bpf_cred_storage_get(struct bpf_map *map,
                          struct cred *cred,
                          void *init,
                          int init__sz,
                          __u64 flags);
...
int cred_storage_get_result = -1; // <-- Added for new test
...
int BPF_PROG(cred_prepare, struct cred *new, const struct cred *old, gfp_t gfp)
{
    ...
    struct bpf_dynptr ret_dynptr;
    ret_dynptr = bpf_cred_storage_get((struct bpf_map *)&cred_storage_map,
                      new,
                      NULL,
                      0,
                      BPF_LOCAL_STORAGE_GET_F_CREATE);
    if (!bpf_dynptr_is_null(&ret_dynptr)) {
        __u32 get_value = 0;
        if (bpf_dynptr_read(&get_value, sizeof(get_value), &ret_dynptr, 8, 0) == 0 &&
            get_value == DUMMY_STORAGE_VALUE)
